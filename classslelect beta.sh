...
# ===================== 版本更新说明 =====================
# v5.4 
# 主要修复问题：彻底解决密码系统失效的所有根因（共6项修复）
# 根因1（致命）: 子shell陷阱 — $()中exit只终止子shell不终止主脚本，
#        哈希计算失败时空值被写入密码文件，且无任何报错
# 根因2（致命）: read_passwd中echo ""输出到stdout，被$()捕获后
#        导致密码字符串前缀多一个换行，哈希值完全不匹配
# 根因3（严重）: echo -n跨平台行为不一致，macOS/Linux存在差异
# 根因4（严重）: HASH_CMD用字符串存储，参数拆分可能异常
# 根因5（中等）: 密码文件损坏后无法自动恢复
# 根因6（轻微）: 函数定义在调用之后，依赖脚本整体加载才能执行
#
# 修复措施：
# 1. 改用全局变量_HASH_RESULT传递哈希结果，彻底杜绝子shell问题
# 2. read_passwd中UI换行输出改到stderr，不污染返回值
# 3. 用printf '%s'替代echo -n，跨平台零差异
# 4. 用数组HASH_CMD_ARRAY存储命令，参数严格分离
# 5. 新增密码文件完整性校验，损坏自动重建
# 6. 密码系统模块整体前置，函数先定义后使用
# 7. 脚本密码完全独立，与操作系统用户密码无任何关联
